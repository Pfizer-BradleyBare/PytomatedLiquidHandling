...
def Callback(step):
	...
	HAMILTONIO.AddCommand(STATUS_UPDATE.AddProgressDetail({"DetailMessage": "Starting " + NumtoWordDict[str(RepsCompleted)] + " followup of Magnetic Beads Block. Block Coordinates: " + str(step.GetCoordinates())}),False,True)
	...
	if DoRemove == True:
		...
		HAMILTONIO.AddCommand(STATUS_UPDATE.AddProgressDetail({"DetailMessage": "Removing all liquid from beads plate"}),False,True)
	...
	if DoAdd == True:
		...
		HAMILTONIO.AddCommand(STATUS_UPDATE.AddProgressDetail({"DetailMessage": "Adding storage buffer to beads plate and mixing vigorously to resuspend beads"}),False,True)
		HAMILTONIO.AddCommand(PIPETTE.Transfer(TransferArgumentsDict))
		Response = HAMILTONIO.SendCommands()
	...
	if RepsCompleted < Reps:
		...
		HAMILTONIO.AddCommand(STATUS_UPDATE.AddProgressDetail({"DetailMessage": "Moving beads plate to magnetic rack"}),False,True)
		...
		HAMILTONIO.AddCommand(STATUS_UPDATE.AddProgressDetail({"DetailMessage": "Adding block to timer list to condense beads for " + str(Time) + " minutes"}),False,True)
		HAMILTONIO.AddCommand(STATUS_UPDATE.AddProgressDetail({"DetailMessage": "Ending " + NumtoWordDict[str(RepsCompleted)] + " followup of Magnetic Beads Block. Block Coordinates: " + str(step.GetCoordinates())}),False,True)
		Response = HAMILTONIO.SendCommands()
		#Move plate to rack
...
def Step(step):
	HAMILTONIO.AddCommand(STATUS_UPDATE.AddProgressDetail({"DetailMessage": "Starting Magnetic Beads Block. Block Coordinates: " + str(step.GetCoordinates())}),False,True)
	HAMILTONIO.SendCommands()
	...
	HAMILTONIO.AddCommand(TRANSPORT.MoveLabware({"SourceLabwareType":PlateType,"SourceSequenceString":PlateSequence,"DestinationLabwareType":RackType,"DestinationSequenceString":RackSequence,"Park":"True","CheckExists":"After"}))
	HAMILTONIO.AddCommand(STATUS_UPDATE.AddProgressDetail({"DetailMessage": "Adding block to timer list to condense beads for " + str(Time) + " minutes"}),False,True)
	HAMILTONIO.AddCommand(STATUS_UPDATE.AddProgressDetail({"DetailMessage": "Ending Magnetic Beads Block. Block Coordinates: " + str(step.GetCoordinates())}),False,True)
	Response = HAMILTONIO.SendCommands()
	#Move plate to rack
