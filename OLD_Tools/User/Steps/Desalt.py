...
def Equilibrate(ParentPlate):
	...
	if Params["EQ"] == False:
		Params["EQ"] = True
		HAMILTONIO.AddCommand(STATUS_UPDATE.AddProgressDetail({"DetailMessage": "Performing IMCS SizeX desalting equilibration"}),False,True)
		HAMILTONIO.AddCommand(DESALT.Equilibrate({"ParentPlate":ParentPlate, "StartPosition":SAMPLES.StartPosition}))
	else:
		HAMILTONIO.AddCommand(STATUS_UPDATE.AddProgressDetail({"DetailMessage": "IMCS SizeX desalting equilibration was already performed"}),False,True)
...
def Process(ParentPlate):
	...
	Params["EQ"] = False
	HAMILTONIO.AddCommand(STATUS_UPDATE.AddProgressDetail({"DetailMessage": "Performing IMCS SizeX desalting"}),False,True)
...
#########################################################################	
def Step(step):
	HAMILTONIO.AddCommand(STATUS_UPDATE.AddProgressDetail({"DetailMessage": "Starting IMCS SizeX Desalting Block. Block Coordinates: " + str(step.GetCoordinates())}),False,True)
	HAMILTONIO.SendCommands()
	...
	HAMILTONIO.AddCommand(STATUS_UPDATE.AddProgressDetail({"DetailMessage": "Ending IMCS SizeX Desalting Block. Block Coordinates: " + str(step.GetCoordinates())}),False,True)
	HAMILTONIO.SendCommands()
